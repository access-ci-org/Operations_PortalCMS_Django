...
@login_required
@permission_required('portal.can_review_systemstatusnews', 
                     login_url=reverse_lazy('portal:unprivileged'))
def approve_systemstatus_news(request, pk):
    """Approve system status news"""
    news = get_object_or_404(SystemStatusNews, pk=pk)
    
    if not can_approve_or_reject(news, request.user, 'portal.can_review_systemstatusnews'):
...
def reject_systemstatus_news(request, pk):
    ...
    return redirect('portal:update_system_status_news', pk=pk)


@login_required
@permission_required('portal.can_publish_systemstatusnews',
                     login_url=reverse_lazy('portal:unprivileged'))
def publish_systemstatus_news(request, pk):
    """Publish approved system status news"""
    news = get_object_or_404(SystemStatusNews, pk=pk)
    
    if not can_publish(news, request.user, 'portal.can_publish_systemstatusnews'):
...
@login_required
@permission_required('portal.can_review_integrationnews',
                     login_url=reverse_lazy('portal:unprivileged'))
def approve_integration_news(request, pk):
    """Approve integration news"""
    news = get_object_or_404(IntegrationNews, pk=pk)
    
    if not can_approve_or_reject(news, request.user, 'portal.can_review_integrationnews'):
        messages.error(request, 'You cannot approve this news.')
        return redirect('portal:integration_news')
...
def reject_integration_news(request, pk):
    ...
    return redirect('portal:update_integration_news', pk=pk)


@login_required
@permission_required('portal.can_publish_integrationnews',
                     login_url=reverse_lazy('portal:unprivileged'))
def publish_integration_news(request, pk):
    """Publish approved integration news"""
    news = get_object_or_404(IntegrationNews, pk=pk)
    
    if not can_publish(news, request.user, 'portal.can_publish_integrationnews'):
        messages.error(request, 'You cannot publish this news.')
        return redirect('portal:integration_news')
